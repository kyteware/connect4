...
fn main() {
    let board = Position::default()
        .with_move(Color::Red, 3)
        .with_move(Color::Yellow, 0)
        .with_move(Color::Red, 2)
        // .with_move(Color::Yellow, 0)
        // .with_move(Color::Red, 4)
        .with_move(Color::Yellow, 0);

    println!("best move: {}", minmax(&board, Color::Red, 2).0);
}

fn minmax(board: &Position, turn: Color, depth: u32) -> (usize, f64) {
    ...
    for mve in board.allowed_moves() {
        ...
        let score = if depth > 0 {
            if favour == std::f64::INFINITY * turn.favour_mod() {
                println!("ah ok {:?} {}", turn, depth);
                return (mve, favour)
            }
            minmax(&board.with_move(turn, mve), turn.opposite(), depth - 1).1
        } else {
            favour
        };

        if score > best.1 * turn.favour_mod() {
            println!("{:?} is better than {:?}", (mve, score), best);
            best = (mve, score);
        }
    }
